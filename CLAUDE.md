...
- ✅ P-002: PDF処理実行ページ（ファイルアップロード、処理完了）
- ✅ P-003: 処理履歴・ダウンロードページ（ダウンロード、フィルター）
...
- `docs/SCOPE_PROGRESS.md`: 開発進捗状況（**クリーンアップ禁止** - 開発全体の記録）
- `README.md`: プロジェクト概要・セットアップ手順

### 7.2 E2Eテスト関連ドキュメント（Phase 9）

**重要**: E2Eテスト進捗は専用ファイルで管理。SCOPE_PROGRESS.mdをクリーンアップしないこと。

| ファイル | 用途 |
|---------|------|
| `docs/e2e-test-checklist.md` | **E2Eテスト進捗チェックリスト**（メイン進捗管理ファイル） |
| `docs/e2e-specs/*-e2e.md` | 各ページのE2Eテスト仕様書（auth, process, history, users, companies） |
| `docs/e2e-best-practices.md` | E2Eテスト成功パターンの蓄積 |
| `docs/e2e-test-history/` | テスト履歴（passed-tests.md, debug-sessions.md, manual-checks.md） |
| `playwright.config.ts` | Playwright設定（headless: true, open: 'never'） |
| `tests/e2e/` | E2Eテストコード |
...
**最終更新**: 2025-12-09
**バージョン**: 1.4
**Phase 8（API統合）完了 → Phase 9（E2Eテスト）待機中**

### 変更履歴

- **v1.4** (2025-12-09): Phase 8完了に伴う更新
  - API統合完了（全4ページ）
  - モックサービスを削除、APIクライアントに置き換え
  - Excel検証機能追加（AA17/AA19、動的行、自動計算チェック）
  - 処理順序最適化（検証→PDF生成）
- **v1.3** (2025-12-04): Phase 4 UI改善・E2Eテスト仕様書更新
  - P-002: ナビゲーション警告ダイアログ追加（サイドバー・戻るボタン両方をブロック）
  - 全ページ: レスポンシブUI改善（ダイアログボタン縦並び、情報表示コンパクト化）
  - E2Eテスト仕様書を現在の実装に合わせて更新
- **v1.2** (2025-12-03): Phase 4完了に伴う更新
  - ページ構成を「全7ページ」に修正（全ページ完成）
  - P-004保護機能に「自分自身のロール変更禁止」を追加
  - 通知UI適用済みページにP-002, P-003を追加
