...
| オフ・ビート・ワークス | `offbeat-to-terra` を含む | 種別ごとに異なる（下記参照） |
...
**将来の拡張候補**:
- 2要素認証（2FA）: Google Authenticator等によるワンタイムパスワード（運用開始後、必要に応じて追加）
...
| Phase 11 | PDF出力最適化 + Electron版機能準備 | Excel直接PDF出力対応、ユーザー管理機能Electron対応 |
| Phase 12 | Electron化（デスクトップアプリ） | Webアプリを.exe形式にパッケージング |

### Phase 11 詳細: PDF出力最適化 + Electron版機能準備

**目的**: PDF出力品質の向上と、Electron版に必要な機能の実装

**Phase 11-1: PDF出力エンジン切り替え**
- 環境変数（PDF_ENGINE）による出力方式切り替え
- LibreOffice版（AWS Lambda用）とExcel直接出力版（社内用）の併用
- WSL2からWindows側のExcel（ExportAsFixedFormat）を呼び出してPDF生成
- ロールバック可能な設計（環境変数変更のみで切り替え）

**Phase 11-2: ユーザー管理機能Electron対応**
- 環境変数（APP_MODE）によるユーザー管理機能の切り替え
- Web版: 招待メール送信、パスワードリセットメール
- Electron版: 管理者による直接ユーザー追加、直接パスワードリセット

**Phase 11-3: 動作確認**
- 単体テスト、結合テスト
- PDF_ENGINE / APP_MODE 切り替えテスト
- PDF品質確認
...
### Phase 12 詳細: Electron化（デスクトップアプリ）

**目的**: 社内配布用デスクトップアプリの作成

**前提**: Phase 11完了後に着手

**内容**:
- Webアプリを.exe形式にパッケージング
- フロントエンド + バックエンド + Python処理を1つのアプリに統合
- 社内ファイルサーバーでの配布（インストール不要のPortable形式）
- 利用者PCの環境要件: Windows 10/11 + Microsoft Excel 2016以降

**詳細**: `docs/PDF出力改善_デスクトップアプリ化_計画.md` を参照
...
| 日付 | バージョン | 変更内容 | 変更者 |
|------|-----------|---------|--------|
| 2026-01-15 | 2.6 | Phase 11/12構成を整理<br>- Phase 11: PDF出力最適化 + Electron版機能準備（11-1〜11-5の5段階）<br>- Phase 12: Electron化（別フェーズとして分離）<br>- SCOPE_PROGRESS.md・計画書との整合性を確保 | - |
| 2026-01-15 | 2.5 | Phase 11-1にAPP_MODE（ユーザー管理機能の切り替え）を追加、計画書ファイル名を修正 | - |
| 2026-01-09 | 2.4 | Phase 11（PDF出力最適化 + デスクトップアプリ化）を追加<br>- Phase 11-1: PDF出力エンジン切り替え対応（LibreOffice/Excel直接出力）<br>- Phase 11-2: Electron化（.exe形式での社内配布）<br>- 詳細は`docs/PDF出力改善_デスクトップアプリ化_計画.md`を参照 | - |
| 2025-12-04 | 2.3 | レスポンシブUI改善・ナビゲーション制御追加<br>- P-002: 処理中のナビゲーション警告ダイアログ追加（サイドバー・戻るボタン両方をブロック）<br>- P-002: レスポンシブ対応改善（取引先表示の段落分け、アラートメッセージの折り返し、フォントサイズ調整）<br>- P-002: コンテンツの見切れ修正（overflow-x-hidden対応）<br>- P-003: 処理詳細モーダルのタイトルから会社名を削除（冗長なため）<br>- P-003: 処理詳細モーダルの情報表示をコンパクト化（ラベルと値を横並びに）<br>- P-004: ユーザー削除ダイアログのレスポンシブ対応（ボタン縦並び）<br>- P-005: 取引先詳細ダイアログにタイトル「取引先詳細」追加<br>- P-005: ダイアログボタンのレスポンシブブレークポイント修正（lg→sm）<br>- 全ページ: ページタイトルの配置・スタイル統一<br>- 共通: サイドバーナビゲーション制御用Zustandストア追加<br>- 理由: モバイル・タブレット環境でのユーザビリティ向上 | - |
...
| 2025-12-01 | 1.8 | P-002機能詳細化（Phase 4実装による仕様反映）<br>- ページ構成: 「全8ページ」→「全7ページ」に修正<br>- ページ構成: 「メイン機能ページ（5ページ）」→「メイン機能ページ（4ページ）」に修正<br>- P-002: PDFアップロード方式を追加（まとめて/個別アップロード）<br>- P-002: スロット管理機能を追加（4種類のPDFスロットで不足ファイル表示）<br>- P-002: PDF種別判別ルールを追加（ファイル名キーワードによる自動判別）<br>- P-002: 「請書」キーワードを「請求」より優先判定に設定<br>- P-002: 個別スロットへのアップロード時の種別検証を追加<br>- P-002: 取引先判別ルール（命名規則）を追加（ネクストビッツ様・オフ・ビート・ワークス様）<br>- P-002: 取引先混在検知機能を追加<br>- P-002: ドラッグ&ドロップUI仕様を追加（オーバーレイ表示）<br>- P-002: エラー表示ルールを追加（Snackbar/ダイアログの使い分け）<br>- P-002: 画面遷移に不足ファイル時の分岐を追加<br>- 理由: ユーザビリティ向上と操作性改善のため、および実装との整合性確保 | - |
