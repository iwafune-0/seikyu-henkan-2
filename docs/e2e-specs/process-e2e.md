...
前提条件:
  ...
  - テスト用PDFファイル4つ準備（tests/e2e/fixtures/nextbits/）
    - TRR-25-007_お見積書.pdf
    - TRR-25-007_請求書.pdf
    - TRR-25-007_注文請書.pdf
    - TRR-25-007_納品書.pdf
...
手順:
  ...
  6. テンプレートExcel（テラ【株式会社ネクストビッツ御中】注文検収書_2506.xlsx）をアップロード
  7. 「処理を実行」ボタンをクリック
...
```yaml
前提条件:
  - テスト用PDFファイル4つ準備（tests/e2e/fixtures/offbeat/）
    - 1951023-見積-offbeat-to-terra-202507.pdf
    - 2951023-請求_offbeat-to-terra-202507.pdf
    - 請書_offbeat-to-terra-202507.pdf
    - 3951023-納品-offbeat-to-terra-202507.pdf

手順:
  1. /process にアクセス
  2. 4つのPDFをアップロード
  3. 取引先「株式会社オフ・ビート・ワークス」が自動判別されることを確認
...
```yaml
前提条件:
  - ネクストビッツのPDF2つ + オフ・ビート・ワークスのPDF2つ
...
期待結果:
  - ダイアログで「ファイル名から判別されたPDF種別と指定されたスロットが一致しません」エラー表示
```
...
期待結果:
  - ダイアログで「Excelファイル（.xlsx または .xls）を選択してください」エラー表示
```
...
期待結果:
  - ダイアログで「選択中の取引先と異なるExcelファイルがアップロードされました」エラー表示
```
...
手順:
  1. オフ・ビート・ワークスのExcelファイルをアップロード
...
期待結果:
  - ダイアログで「ファイルサイズは10MB以下にしてください」エラー表示
```
...
期待結果:
  - 「このページから移動しますか？」ダイアログが表示される
  - 「キャンセル」ボタンが表示される
  - 「このページから移動する」ボタンが表示される
```
...
手順:
  1. ダイアログの「キャンセル」ボタンをクリック

期待結果:
  - ダイアログが閉じる
  - 処理ページに留まる
  - アップロードしたファイルが保持されている
```
...
| ファイル名 | 種別 |
|-----------|------|
| TRR-25-007_お見積書.pdf | 見積書 |
| TRR-25-007_請求書.pdf | 請求書 |
| TRR-25-007_注文請書.pdf | 注文請書 |
| TRR-25-007_納品書.pdf | 納品書 |
| テラ【株式会社ネクストビッツ御中】注文検収書_2506.xlsx | テンプレート |

### オフ・ビート・ワークス用テストファイル
| ファイル名 | 種別 |
|-----------|------|
| 1951023-見積-offbeat-to-terra-202507.pdf | 見積書 |
| 2951023-請求_offbeat-to-terra-202507.pdf | 請求書 |
| 請書_offbeat-to-terra-202507.pdf | 注文請書 |
| 3951023-納品-offbeat-to-terra-202507.pdf | 納品書 |
| テラ【株式会社オフ・ビート・ワークス御中】注文検収書_2506.xlsx | テンプレート |

### テストデータ配置場所
```
tests/e2e/fixtures/nextbits/
tests/e2e/fixtures/offbeat/
```
