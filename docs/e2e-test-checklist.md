...
### E2E-USER-003/014: Supabaseメール送信レート制限

**発生日**: 2026-01-27

**問題の概要**:
E2E-USER-003（管理者として招待）とE2E-USER-014（削除済みユーザーへの再招待）がSupabaseのメール送信レート制限により実行できない。
...
**症状**:
- テスト実行中にSupabaseのメール送信制限（無料プラン: 1時間あたり2〜4通）に達する
- 招待メール送信APIがレート制限エラーを返す

**補足**:
- これらは`APP_MODE=web`（招待メール方式）用のテスト
- `APP_MODE=electron`（直接追加方式）ではメール送信しないため、この制限は関係ない
- 実装自体は正常に動作する（手動テストで確認済み）

**現在の対応**:
- E2E-USER-003/014をスキップ（`[-]`マーク）
- SMTP設定完了後に再実行予定

**優先度**: 低（SMTP設定後に対応、実装は正常）
